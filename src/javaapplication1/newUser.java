...
public class newUser extends javax.swing.JFrame {
    ...
    private void newUserAction(){
        if(this.no.getText().length()==0 || this.name.getText().length()==0 || this.phone.getText().length() == 0){
            JOptionPane.showMessageDialog(null, "<html><b>有字段没有填写</b><br>在此操作中，所有字段都被要求填写。</html>", "新建管理员失败", JOptionPane.ERROR_MESSAGE);
            return;
        }
        db con=new db();
        ResultSet test=con.cha("select * from libuser where id="+this.no.getText());
        if(test==null){
        ...
        try{
            if(!test.next()){
                ...
                con.runSql("INSERT INTO libuser ( id,name,phone,allowborrow,isforzed) VALUES (  "+this.no.getText()+", '"+this.name.getText()+"', "+this.phone.getText()+", 5, 0)");
                JOptionPane.showMessageDialog(null, "<html>已添加用户</html>", "完成", JOptionPane.INFORMATION_MESSAGE);
                aa.listUser();
                this.dispose();
            }else{
